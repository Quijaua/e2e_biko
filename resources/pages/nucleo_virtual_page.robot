...
*** Keywords ***
 ...
 Verificar se está na página de cadastro de aula virtual
    Wait Until Element Is Visible    ${ADD_CLASS_TITLE}    timeout=10s
...
Clicar no botão Salvar do cadastro de aula
    Wait Until Element Is Visible    ${ADD_CLASS_SUBMIT_BTN}    timeout=10s
    Wait Until Element Is Enabled    ${ADD_CLASS_SUBMIT_BTN}    timeout=10s
...
Verificar mensagem de sucesso de aula criada
   Wait Until Element Is Visible    //div[contains(@class, 'alert-success') and contains(., 'Aula virtual criada com sucesso!')]    timeout=10s
...
Verificar card da aula criada
   ...
   Wait Until Element Is Visible    ${card_xpath}    timeout=10s
...
Clicar no botão excluir da aula criada
   ...
   Wait Until Element Is Visible    ${excluir_btn_xpath}    timeout=10s
   Click Element                    ${excluir_btn_xpath}

Verificar mensagem de sucesso de aula excluída
   Wait Until Element Is Visible    //div[contains(@class, 'alert-success') and contains(., 'Aula virtual excluida com sucesso!')]    timeout=10s
...
Editar aula criada
   ...
   Wait Until Element Is Visible    ${editar_btn_xpath}    timeout=10s
   Click Element                    ${editar_btn_xpath}
   Wait Until Element Is Visible    ${ADD_CLASS_FORM}    timeout=10s
...
Verificar mensagem de sucesso de aula editada
   Wait Until Element Is Visible    //div[contains(@class, 'alert-success') and contains(., 'Aula virtual atualizada com sucesso!')]    timeout=10s
...
Clicar no filtro de área
   ...
   Wait Until Element Is Visible    ${filtro_xpath}    timeout=10s
